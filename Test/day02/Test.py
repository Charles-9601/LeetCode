...
class GameField(object):
    # 初始化信息
    def __init__(self, width=8, height=8, win_value=2048):
        self.width = width
        self.height = height
        self.win_value = win_value  # 获胜目标
        self.score = 0  # 当前得分
    ...
    def draw(self, print_screen):
        def draw_sep():
            print_screen.addstr('+' + "-----+" * self.width + '\n')

        def draw_one_row(row):
            print_screen.addstr("".join('|{:^5}'.format(num) if num != 0 else "|     " for num in row) + '|' + '\n')

        # 清屏
        print_screen.clear()
        print_screen.addstr("2048".center(50, '-') + '\n')
        print_screen.addstr("当前分数:" + str(self.score) + '\n')
        if self.high_score != 0:
            print_screen.addstr("最高分:" + str(self.high_score) + '\n')
        ...
        # 判断是否赢或者输
        if self.is_win():
            print_screen.addstr("胜利!!!!" + '\n')
        if self.is_gameover():
            print_screen.addstr("游戏结束!!!!" + '\n')
        print_screen.addstr(" 游戏帮助: 上下左右键  (R)Restart     Q(Quit)")
